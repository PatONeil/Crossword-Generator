class ReviewTab {
  ...
  init() {
    this.reviewWords = this.model.words;
  ...
  showCurrentWord() {
    ...
    this.renderDefs(item);
  }
  renderDefs(word) {
    this.defsList.innerHTML = "";
    const entry = this.model.dictionary[word.word];
    ...
    defs.forEach((def, i) => {
      ...
      radio.type = "radio";
      radio.name = `def-select-${word.word}`;
      radio.value = def;
      if (def === word.definition) radio.checked = true;
      radio.addEventListener("click", function(e) {
        if (e.target.checked) word.definition = e.target.value;
      });
